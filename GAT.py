...
class GATLayerImp2(GATLayer):
    ...
    def forward(self, data):
        ...
        b = in_nodes_features.shape[0]
        assert connectivity_mask.shape[1:] == (num_of_nodes, num_of_nodes), \
        ...
        # shape = (N, FIN) * (FIN, NH*FOUT) -> (N, NH, FOUT) where NH - number of heads, FOUT - num of output features
        # We project the input node features into NH independent output features (one for each attention head)
        nodes_features_proj = self.linear_proj(in_nodes_features).view(b, -1, self.num_of_heads, self.num_out_features)
        ...
        all_attention_coefficients = self.softmax(all_scores.squeeze(1) + connectivity_mask)
        ...
        out_nodes_features = torch.bmm(all_attention_coefficients, nodes_features_proj.squeeze(2))
        ...
        out_nodes_features = self.skip_concat_bias(all_attention_coefficients, in_nodes_features, out_nodes_features.permute(1,0,2).unsqueeze(1))
